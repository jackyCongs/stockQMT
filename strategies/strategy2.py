...
class Strategy2:
    def __init__(self, db, trader_service, platform):
        ...
        # 单个股票最大可以持仓多少钱
        self.max_single_amount = 3000
        # 每次出价最低多少钱
        self.min_bid_amount = 300

        #todo 这里为了方便测试有更多数据设置了0，开放测试后要设置回来
        self.base_premium_threshold = 0.38
    ...
    def handler(self, msgs):
        ...
        try:
            if not utils.is_market_after_buffer():
                #print("未到开盘时间或已收盘")
                return
            target_index = None
            for code in list(msgs):
                ...
                if asset.cash >= self.min_bid_amount:
                    if first_sell_queue_node is not None and first_sell_queue_node.code == code and self.is_max_premium(code):
